/**
该包主要实现web的类似java的session功能
实现思想：
   session作用是以键值对的形式保存一些数据，
   这些数据在客户端和服务器都有一份，必要时进行比对
   基于每个请求都携带cookie数据，所以，就直接将session的唯一
   标示存入到cookie中，再将cookie写到响应中。这样既可以减少每次请求携带cookie的数据量
   亦可以保证客户端和服务器session的一致。验证时根据该唯一标示在后台服务器查找
   找到后进行数据操作。同一个应用有多个session，同一个session有多个request,必须充分考虑并发问题
   考虑到速度问题，尽可能的将锁级别粒度细化，就是尽可能的在多处上锁，而避免在粗粒度大范围内上锁
   但是锁太多，也会损失资源，需要权衡
*/
